...
public final class GameController {
    ...
    public void spawnNewPiece() {
        ...
        lastSpawnCol=startCol;
        if(!gameOver && aiEnabled) ai.onNewPiece(board.copy(), current.type(), lastSpawnCol);
    ...
    public void tick(long now) {
        ...
        if(!paused && !gameOver && aiEnabled){
            Command c=ai.next(current);
            switch(c){
                case LEFT -> tryMoveLeft();
                case RIGHT -> tryMoveRight();
                case ROTATE -> tryRotate();
                case DROP -> { while(tryBoost()); lockPiece(); }
                case NONE -> {}
            }
        }
    }
